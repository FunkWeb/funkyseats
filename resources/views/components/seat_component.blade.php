<div class='text-center text-light text-uppercase' onClick='bookSeat()'>
    ...
    @elseif (Auth::check() && count($seat->booking) == 2 && ((string)Auth::user()->id == $seat->booking[0]->user_id ||
            (string)Auth::user()->id == $seat->booking[1]->user_id))
        <div class='booked-by-me seat-container'>
    @elseif (Auth::check() && count($seat->booking) == 1 &&
            (\Carbon\Carbon::parse($seat->booking[0]->from)->format('H') == 12 || \Carbon\Carbon::parse($seat->booking[0]->from)->format('H') == 8))
